			...
			elseif ( strstr( $key, 'attribute:' ) ) {
				...
				if ( substr( $attribute_key, 0, 3 ) == 'pa_' ) {
					...
					if ( ! taxonomy_exists( $taxonomy ) ) {
						...
						$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __('> > Attribute taxonomy "%s" does not exist. Adding it. Nicename: %s', 'wc_csv_import'), $taxonomy, $nicename ) );
						...
						if ( ! $exists_in_db ) {
							...
							$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __('> > Attribute taxonomy %s already exists in DB.', 'wc_csv_import'), $taxonomy ) );
						}
						...
						foreach ( $raw_terms as $raw_term ) {
							...
							if ( ! $term_id ) {
								...
								if ( ! is_wp_error( $t ) ) {
									...
									$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __( '> > Inserted Raw Term %s ID = %s', 'wc_csv_import' ), esc_html( $raw_term ), $term_id ) );
								} else {
									$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __( '> > Failed to import term %s %s', 'wc_csv_import' ), esc_html($raw_term), esc_html($taxonomy) ) );
									break;
								}
							} else {
								$WC_CSV_Product_Import->log->add( 'csv-import', sprintf( __( '> > Raw Term %s ID = %s', 'wc_csv_import' ), esc_html( $raw_term ), $term_id ) );
							}
